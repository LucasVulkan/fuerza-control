...
| [program-templates.md](program-templates.md) — Programas por plantilla flexible | **fases 1-2 implementadas** (resolvedor de slots, escalera de compresión); 3-8 pendientes: 3 volumen semanal · 3b vincular lo repetido · 4 fases y duración · 5 matcher por ranking · 6 onboarding de propuestas · 7 reglas de integridad · 8 catálogo | 🟡/🔴 | el onboarding **propone programas** en vez de fabricarlos: 4 preguntas → 3 candidatos con su duración y su coste de adaptación → 2 preguntas. **Adaptar sí, planificar no**: el eje "días" se resuelve eligiendo otra plantilla (el modelo es de ciclos rotativos, no de semanas), nunca reorganizando la elegida — un solver acabaría siendo el generador procedural otra vez. Hoy quien pide 1, 2, 5, 6 o 7 días **nunca** ve una plantilla, no hay ninguna PPL pese a ser lo que la app recomienda, y `findSubstitute` devuelve `candidates[0]` sin ordenar. Las fases 1-5 aportan valor sin tocar la UI |
