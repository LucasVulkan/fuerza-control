...
| [onboarding-proposals.md](onboarding-proposals.md) — Onboarding de propuestas | spec cerrada, SIN implementar (ago 2026). Es la **fase 6** de program-templates, extraída a documento propio | 🔴 | La única fase con UI: el onboarding pasa de 8 preguntas y un programa impuesto a **4 preguntas → 3 programas reales a elegir → 2 preguntas de ajuste**. Autocontenida para ejecutarla en frío: estado del código, formas exactas de `rankArchetypes` y `adaptArchetype`, i18n y trampas. **Arrastra la migración de `OnboardingScreen` a FormaFit** — leer UI-MIGRATION.md antes |
