...
- El color rojo en las pills de rep@RPE parece indicar series "al fallo" o de alta dificultad (posible antecedente de un futuro indicador de RPE/esfuerzo) — vale la pena comparar con lo encontrado en Progress. → RESUELTO: el usuario decidió que en la app "fuera de rango" usa la familia NARANJA (patrón tint-30 de fondo + sólido en texto), no roja; "en rango" usa accent (tint-10 fondo). No se replica el rojo de Figma aquí.
- El ícono de "Delete" (X) está en cada card de historial — implica que el historial permite borrar sesiones individuales directamente desde la lista, algo no estándar en apps de fitness (normalmente el historial es de solo lectura). → RESUELTO: confirmado, la app ya lo tenía (X por tarjeta con Alert de confirmación).
- El segmented control "Programa activo / Todos" sugiere que el historial se puede filtrar por si la sesión pertenece al programa activo o no — confirmar si esto ya existe en la app actual. → RESUELTO: confirmado, implementado con el SegmentedControl nuevo.
- Solo una de las 4 Sesion Card está expandida con detalle; no está claro en el mockup si el expand/collapse es interactivo (tap) o si son dos variantes de card distintas (colapsada vs expandida) coexistiendo como ejemplo visual. → RESUELTO: es interactivo (tap en la cabecera de la tarjeta), implementado con acordeón animado.
